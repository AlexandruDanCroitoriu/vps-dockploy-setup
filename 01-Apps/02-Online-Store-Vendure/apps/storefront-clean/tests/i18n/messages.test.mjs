...
test('message keys match across locales', async () => {
    ...
    for (const englishFile of englishFiles) {
        const romanianFile = path.join(path.dirname(englishFile), 'ro.json');
        assert.ok(discovered.includes(romanianFile), `${englishFile} has no matching ro.json file.`);
        const english = JSON.parse(await readFile(path.join(root, englishFile), 'utf8'));
        const romanian = JSON.parse(await readFile(path.join(root, romanianFile), 'utf8'));
        assert.deepEqual(
            messageKeys(romanian).sort(),
            messageKeys(english).sort(),
            `${englishFile} and ${romanianFile} must define the same message keys.`,
        );
    }
});
