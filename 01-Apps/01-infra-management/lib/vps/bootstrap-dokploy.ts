...
export async function runDokployBootstrapStep(
  input: BootstrapDokployInput,
  ...
  try {
    ...
    if (input.step === "installing") {
      await runStep("installing", async () => {
        await runSshCommand(
          input,
        ...
        await waitForUrl(ipUrl, 2 * 60 * 1_000, (message) =>
          onLog("installing", message),
        );
      });
    }
    const authenticate = async (create: boolean) => {
      try {
        if (!create) throw new Error("Sign in");
    ...
    const needsAuthentication = ["administrator", "domain", "api-key"].includes(
      input.step,
    );
    const authentication = needsAuthentication
      ? await (input.step === "administrator"
          ? runStep("administrator", () => authenticate(true))
          : authenticate(false))
      : null;
    const cookie = authentication?.response.headers
      ...
      .join("; ");
    if (needsAuthentication && !cookie)
      throw new Error("Dokploy did not create an admin session.");

    const host = `dockploy.${input.rootDomain}`;
    const rootUrl = `https://${host}`;
    if (input.step === "domain")
    ...
    const generatedApiKey =
      input.step === "api-key"
        ? await runStep("api-key", async () => {
            ...
            const tokenResult = await dokployRequest(
              ipUrl,
            ...
            if (!apiKey)
              throw new Error("Dokploy did not return an API/CLI key.");
            await onApiKey(apiKey);
            await dokployRequest(ipUrl, "/api/project.all", undefined, {
              apiKey,
              method: "GET",
            });
            return apiKey;
        ...
        : null;
    const apiKey = generatedApiKey ?? input.apiKey ?? "";
